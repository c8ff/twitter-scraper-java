...
public class TestUserTweetsReplies {
	public static void main(String[] args) {
		TwitterApi api = TwitterApi.newTwitterApi();
		Secret.defineFromFile(api);

		try (CloseableHttpClient client = HttpClients.createDefault()) {
			UserTweetsReplies result = api.scrap(new ConfigUserTweetsReplies("783214"), client);
			System.out.println(result);
			Files.writeString(new File("user-tweets-replies.json").toPath(), JsonUtil.toJson(result));
