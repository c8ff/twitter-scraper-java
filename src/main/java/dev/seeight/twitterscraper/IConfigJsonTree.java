...
public interface IConfigJsonTree<T> extends IConfig<T> {
	...
	static JsonElement assertErrors(@NotNull JsonElement elm, @Nullable Request request, @Nullable List<TwitterError> errorList) throws TwitterException {
		...
		for (TwitterError err : errorList) {
			if (err.code == TwitterError.ALREADY_RETWEETED
                    || err.code == TwitterError.ALREADY_FAVORITED
                    || err.code == TwitterError.ALREADY_UNFAVORITED
                    || err.code == TwitterError.TIMEOUT_UNSPECIFIED
                    || err.code == TwitterError.DECODE_EXCEPTION
            ) continue;
