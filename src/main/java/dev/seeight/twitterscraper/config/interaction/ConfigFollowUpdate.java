...
public class ConfigFollowUpdate implements IConfigJsonTree<LegacyUser> {
	...
	public ConfigFollowUpdate(String userId, boolean notifications, boolean retweets) {
		this.userId = userId;
	...
	public Request.Builder createRequest(Gson gson, HttpUrl url, TwitterApi api) throws URISyntaxException {
        // TODO: Somehow only send retweets when changing them, not all the time.
        return new Request.Builder().url(url).post(
                new FormBody.Builder()
                        ...
                        .add("skip_status", "1")
                        .add("user_id", userId)
                        .add("device", notifications ? "1" : "0")
                        .add("retweets", retweets ? "1" : "0")
                        .build());
	}
}
