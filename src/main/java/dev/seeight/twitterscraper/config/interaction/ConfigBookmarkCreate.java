...
public class ConfigBookmarkCreate implements IConfigJsonTree<Boolean> {
	...
	public HttpUrl getUrl(Gson gson, TwitterApi api) throws URISyntaxException {
		return api.getGraphQLOperation("CreateBookmark").getBaseUrl();
	}
	...
	public Request.Builder createRequest(Gson gson, HttpUrl url, TwitterApi api) throws URISyntaxException, MalformedURLException {
		var op = api.getGraphQLOperation("CreateBookmark");
	...
	public Boolean fromJson(JsonElement element, Gson gson, List<TwitterError> errors) {
		for (TwitterError error : errors) if (error.code == TwitterError.ALREADY_FAVORITED) return true;
